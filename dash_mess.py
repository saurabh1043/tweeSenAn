...
app.layout = html.Div(
    [   html.Div(className='container-fluid', children=[html.H2('Twitter Sentiment Analyser', style={'color':"#CECECE"}),
                                                        html.H5('Enter Terms:', style={'color':app_colors['text']}),
                                                  dcc.Input(id='sentiment_term', value='gst ', type='text', style={'color':app_colors['someothercolor']}),
                                                  ],
...
@app.callback(Output('recent-tweets-table', 'children'),
              [Input(component_id='sentiment_term', component_property='value'),Input('recent-table-update', 'n_intervals')])
...
def update_recent_tweets(sentiment_term,n_intervals):
    if sentiment_term:
        ...
        df1 = pd.read_sql("SELECT sentiment.* FROM sentiment_fts fts LEFT JOIN sentiment ON fts.rowid = sentiment.id WHERE fts.sentiment_fts MATCH ? ORDER BY fts.rowid DESC", conn, params=(sentiment_term+'*',))        
    else:
        df = pd.read_sql("SELECT * FROM sentiment ORDER BY id DESC, unix DESC LIMIT 10", conn)
        df1 = pd.read_sql("SELECT * FROM sentiment ORDER BY id DESC, unix DESC", conn)
    df['date'] = pd.to_datetime(df['unix'], unit='ms')
    df1['date'] = pd.to_datetime(df1['unix'], unit='ms')

    df = df.drop(['unix','id'], axis=1)
    df = df[['date','tweet','sentiment']]
    df1 = df1.drop(['unix','id'], axis=1)
    df1 = df1[['date','tweet','sentiment']]
...
@app.callback(Output('sentiment-pie', 'figure'),
              [Input(component_id='sentiment_term', component_property='value'),Input('sentiment-pie-update', 'n_intervals')])
...
@app.callback(Output('historical-graph', 'figure'),
              [Input(component_id='sentiment_term', component_property='value'),Input('historical-update', 'n_intervals')])
